...
public final class Model {
    ...
    public static class KineticVertex implements VertexRef {
        ...
        @Override public Vec2 positionAt(double time) {
            if (infFast) return startNode.pos;
            if (stopsAt != null && stopNode != null && time >= stopsAt - STOP_EPS) return stopNode.pos;
            return new Vec2(origin.x() + time * velocity.x(), origin.y() + time * velocity.y());
        }

        @Override public Vec2 velocityAt(double time) {
            if (infFast || (stopsAt != null && time >= stopsAt - STOP_EPS)) return new Vec2(0, 0);
            return velocity;
        }
    ...
    public static class KineticTriangle {
        ...
        public int getType() {
            int noneCt = 0;
            for (var n : neighbours) if (n == null) noneCt++;
            return noneCt;
        }

        public boolean isFinite() {
            for (var v : vertices) if (!(v instanceof KineticVertex)) return false;
            return true;
        }

        public int indexOfVertex(VertexRef v) {
            for (int i = 0; i < 3; i++) if (vertices[i] == v) return i;
            return -1;
        }

        public int indexOfNeighbour(KineticTriangle t) {
            for (int i = 0; i < 3; i++) if (neighbours[i] == t) return i;
            return -1;
        }
    }
    ...
    public static class Skeleton {
        ...
        public List<Segment> segments() {
            ...
            for (var v : vertices) {
                if (v.stopsAt != null) {
                    if (v.startNode == v.stopNode) continue;
