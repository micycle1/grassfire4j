...
import grassfire2.geom.Geom.*;
import grassfire2.model.Model.*;

import java.util.*;
import java.util.function.IntUnaryOperator;

import static grassfire2.core.Core.*;
...
public class Events {
    ...
    public static class EventQueue {
        ...
        public boolean isEmpty() {
            while (!heap.isEmpty() && !heap.peek().valid) heap.poll();
            return heap.isEmpty();
        }
    }
    ...
    public static void stopKVertices(List<KineticVertex> V, int step, double now, Skeleton skel, Vec2 pos) {
        ...
        boolean isNew = false;
        for (var v : V) {
            if (v.stopsAt != null) skNode = v.stopNode;
            else if (nearZero(v.startsAt - now)) skNode = v.startNode;
            else v.stopsAt = now;
        }
        if (skNode == null) {
            if (pos == null) {
        ...
        if (isNew) skel.skNodes.add(skNode);
    }
    ...
    public static KineticVertex computeNewKVertex(WaveFront wfl, WaveFront wfr, double now, SkeletonNode skNode, int info, boolean internal) {
        ...
        kv.velocity = bi;
        if (bi.x() == 0 && bi.y() == 0) { kv.infFast = true; kv.origin = skNode.pos; }
        else kv.origin = pos0;
    ...
    public static List<KineticTriangle> replaceKVertex(KineticTriangle t, KineticVertex v, KineticVertex newv, double now, IntUnaryOperator dir, EventQueue queue, Deque<Event> imm) {
        ...
        while (t != null) {
            ...
            if (newv.infFast && t.event != null) { queue.discard(t.event); imm.remove(t.event); }
            else replaceInQueue(t, now, queue, imm);
    ...
    public static void schedImm(KineticTriangle tri, double now, EventQueue queue, Deque<Event> imm) {
        ...
        if (tri.getType() == 3) e = new Event(now, tri, List.of(0,1,2), "edge", 3);
        tri.event = e;
        imm.addLast(e);
    }
    ...
    public static void updateCirc(VertexRef left, VertexRef right, double now) {
        if (left instanceof KineticVertex) ((KineticVertex) left).setRight(right, now);
        if (right instanceof KineticVertex) ((KineticVertex) right).setLeft(left, now);
    }
    ...
    public static void flip(KineticTriangle t0, int side0, KineticTriangle t1, int side1) {
        ...
        int[] sides = new int[4];
        for (int i=0; i<4; i++) sides[i] = nghs[i] == null ? -1 : ccw(nghs[i].indexOfVertex(cors[i]));

        KineticTriangle[] ts = {t0, t0, t1, t1};
        for (int i=0; i<4; i++) if (nghs[i] != null) nghs[i].neighbours[sides[i]] = ts[i];
    ...
    public static void handleEdge(Event evt, int step, Skeleton skel, EventQueue q, Deque<Event> imm) {
        ...
        if (aTri != null) {
            ...
            if (!fanA.isEmpty()) {
                ...
                if (nearZero(Math.sqrt(orig.distance2At(dest, now)))) schedImm(tLast, now, q, imm);
            }
        }
        if (bTri != null) {
        ...
        if (nTri != null) {
            ...
            if (nTri.event != null && nTri.stopsAt == null) schedImm(nTri, now, q, imm);
        }
        t.stopsAt = now;
    ...
    public static void handleSplit(Event evt, int step, Skeleton skel, EventQueue q, Deque<Event> imm) {
        ...
        if (b != null) b.neighbours[b.indexOfNeighbour(t)] = null;
        List<KineticTriangle> fanB = replaceKVertex(b, v, vb, now, Core::ccw, q, imm);

        KineticTriangle a = t.neighbours[cw(e)];
        if (a != null) a.neighbours[a.indexOfNeighbour(t)] = null;
        List<KineticTriangle> fanA = replaceKVertex(a, v, va, now, Core::cw, q, imm);

        t.stopsAt = now;

        if (va.infFast && !fanA.isEmpty()) handleParallelFan(new ArrayList<>(fanA), va, now, Core::cw, step, skel, q, imm);
        if (vb.infFast && !fanB.isEmpty()) handleParallelFan(new ArrayList<>(fanB), vb, now, Core::ccw, step, skel, q, imm);
    }
    ...
    private static void handleParallelFan(
        ...
        if (fan == null || fan.isEmpty()) throw new IllegalArgumentException("expected fan of triangles");
        if (!pivot.infFast) return;
        ...
        if (firstTri.getType() == 3) {
            ...
            int ct = 0, idx = -1;
            for (int i=0; i<3; i++) if (nearZero(dists[i] - mn)) { ct++; idx = i; }
            if (nearZero(mn) && ct == 1) {
        ...
        // DO NOT compare IntUnaryOperator / method references with '=='.
        // Determine direction by behavior:
        //   cw(0)=2, ccw(0)=1.
        int d0 = direction.applyAsInt(0);
        final boolean isCw;
        if (d0 == 2) isCw = true;
        else if (d0 == 1) isCw = false;
        else throw new IllegalStateException("Unexpected direction operator: op(0)=" + d0);
        ...
        if (leftMin && rightMin) {
            if (fan.size() == 1) {
                ...
                if (all2) {
                    for (KineticTriangle t : fan) handleParallelEdgeEventEvenLegs(t, t.indexOfVertex(pivot), pivot, now, step, skel, q, imm);
                } else {
                    ...
                    if (hasInfFast(t0)) handleParallelEdgeEventEvenLegs(t0, t0.indexOfVertex(pivot), pivot, now, step, skel, q, imm);
                    if (hasInfFast(t1)) handleParallelEdgeEventEvenLegs(t1, t1.indexOfVertex(pivot), pivot, now, step, skel, q, imm);
                }
            } else {
        ...
        if (rightMin) handleParallelEdgeEventShorterLeg(right, rightLegIdx, pivot, now, step, skel, q, imm);
        else handleParallelEdgeEventShorterLeg(left, leftLegIdx, pivot, now, step, skel, q, imm);
    }

    private static boolean hasInfFast(KineticTriangle t) {
        for (VertexRef v : t.vertices) if (v instanceof KineticVertex && ((KineticVertex) v).infFast) return true;
        return false;
    }
    ...
    private static void handleParallelEdgeEventShorterLeg(
        ...
        if (!v1.infFast) toStop.add(v1);
        if (!v2.infFast) toStop.add(v2);
        if (!toStop.isEmpty()) stopKVertices(toStop, step, now, skel, null);

        SkeletonNode skNode = (!toStop.isEmpty()) ? toStop.get(0).stopNode : (pivot.stopNode != null ? pivot.stopNode : pivot.startNode);
        if (skNode == null) return;
        ...
        if (leftRef == null) kv.wfl = null;
        else if (leftRef instanceof KineticVertex) kv.wfl = ((KineticVertex) leftRef).wfr;
        else throw new IllegalStateException("Expected v1.left to be KineticVertex or null");

        VertexRef rightRef = v2.getRight();
        if (rightRef == null) kv.wfr = null;
        else if (rightRef instanceof KineticVertex) kv.wfr = ((KineticVertex) rightRef).wfl;
        else throw new IllegalStateException("Expected v2.right to be KineticVertex or null");
        skel.vertices.add(kv);
        ...
        if (n != null) {
            ...
            if (n.event != null && n.stopsAt == null) schedImm(n, now, q, imm);
        }
        ...
        if (kv.infFast) {
            if (!fanA.isEmpty() && !fanB.isEmpty()) {
                ...
                fan.addAll(fanB);
                handleParallelFan(fan, kv, now, Core::ccw, step, skel, q, imm);
            } else if (!fanA.isEmpty()) handleParallelFan(new ArrayList<>(fanA), kv, now, Core::cw, step, skel, q, imm);
            else if (!fanB.isEmpty()) handleParallelFan(new ArrayList<>(fanB), kv, now, Core::ccw, step, skel, q, imm);
        }
    }
    ...
    public static double eventLoop(EventQueue q, Skeleton skel) {
        ...
        while (!q.isEmpty() || !imm.isEmpty()) {
            if (guard++ > 50_000) throw new RuntimeException("Loop > 50,000");
            step++;
            Event evt = imm.isEmpty() ? q.pop() : imm.pollFirst();
            if (evt == null) continue;
            now = evt.time;

            if (evt.tri.stopsAt != null) continue;
            ...
            if (evt.tp.equals("edge")) {
                if (evt.side.size() == 3) {
                    ...
                    for (var n : evt.tri.neighbours) if (n != null && n.event != null && n.stopsAt == null) {
