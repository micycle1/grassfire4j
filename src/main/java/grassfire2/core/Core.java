...
public final class Core {
    ...
    private static int stepAroundVertex(InputMesh mesh, int vIdx, int cornerId, boolean isCcw) {
        ...
        if (tri.c[edgeSide] != null) return -1;
        int nIdx = tri.n[edgeSide];
        if (nIdx < 0) return -1;
        int[] vArr = mesh.triangles.get(nIdx).v;
        int nSide = -1;
        for (int i = 0; i < 3; i++) if (vArr[i] == vIdx) { nSide = i; break; }
    ...
    public static List<List<List<Corner>>> buildVertexStars(InputMesh mesh) {
        ...
        for (int tIdx = 0; tIdx < triangleCount; tIdx++) {
            ...
            for (int side = 0; side < 3; side++) stars.get(vArr[side]).add(cornerId(tIdx, side));
        }
        ...
        for (int vIdx = 0; vIdx < stars.size(); vIdx++) {
            ...
            for (int c : incidentCorners) unvisitedMark[c] = unvisitedEpoch;
            ...
            while (remaining > 0) {
                ...
                while (true) {
                    ...
                    if (prev < 0 || prev == leftmost || seenMark[prev] == seenEpoch) break;
    ...
    public static Skeleton initSkeleton(InputMesh mesh) {
        ...
        for (int tIdx = 0; tIdx < mesh.triangles.size(); tIdx++) {
            ...
            for (int i = 0; i < 3; i++) {
                if (!mesh.vertices.get(t.v[i]).isFinite) k.vertices[i] = infNodes.get(t.v[i]);
            }
            for (int i = 0; i < 3; i++) {
                if (t.c[i] != null) {
            ...
            for (int i = 0; i < 3; i++) {
                if (t.c[i] == null && t.n[i] != -1) k.neighbours[i] = ktriangles.get(t.n[i]);
            }
        }
        ...
        for (int vIdx = 0; vIdx < stars.size(); vIdx++) {
            if (!mesh.vertices.get(vIdx).isFinite) continue;
        ...
        for (KineticTriangle t : ktriangles) {
            for (int i = 0; i < 3; i++) if (t.vertices[i] instanceof InfiniteVertex) t.vertices[i] = centroid;
        }
