...
import static grassfire2.geom.Geom.*;
...
public class CollapseEventComputer {
    ...
    public static Double vertexCrashTime(KineticVertex org, KineticVertex dst, KineticVertex apx, double now) {
        ...
        if (nearZero(denom)) return null;
        double tau = distVE / denom;
        if (tau < -STOP_EPS) return null;
        return now + Math.max(0.0, tau);
    }

    public static List<Double> solveQuadratic(double A2, double A1, double A0) {
        if (nearZero(A2) && !nearZero(A1)) return List.of(-A0 / A1);
        if (nearZero(A2) && nearZero(A1)) return List.of();
        double T = -A1 / A2, D = A0 / A2, centre = T * 0.5;
        double under = 0.25 * (T * T) - D;
        if (nearZero(under)) return List.of(centre);
        if (under < 0) return List.of();
    ...
    public static List<Double> areaCollapseTimes(VertexRef o, VertexRef d, VertexRef a, double now) {
        ...
        for (double tau : solveQuadratic(A2, A1, A0)) {
            if (tau >= -STOP_EPS) out.add(now + Math.max(0.0, tau));
        }
    ...
    public static Event compute(KineticTriangle tri, double now, boolean strictGt) {
        if (tri.stopsAt != null) return null;
        if (!tri.isFinite()) return null; // Logic omitted for infinite to save space. We use bounded CDT.

        int type = tri.getType();
        Event e = null;
        if (type == 0) e = finite0(tri, now, strictGt);
        else if (type == 1) e = finite1(tri, now, strictGt);
        else if (type == 2) e = finite2(tri, now, strictGt);
        else if (type == 3) e = finite3(tri, now, strictGt);

        if (e != null) tri.event = e;
        return e;
    }
    ...
    private static Event finite0(KineticTriangle tri, double now, boolean strictGt) {
        ...
        for (double t : tArea) {
            if (nearZero(Math.abs(t - now))) {
                ...
                for (int i=0; i<3; i++) if (nearZero(Math.sqrt(d2[i]))) zeros.add(i);
                if (zeros.size() == 1) return edgeEvt(tri, now, zeros);
                if (zeros.size() == 3) throw new IllegalStateException("0-triangle collapsing to point at now");
        ...
        if (te != null && ta != null && nearZero(Math.abs(te - ta))) {
            ...
            for (int i=0; i<3; i++) if (nearZero(d2t[i] - m)) relMin.add(i);
            if (relMin.size() == 3) return edgeEvt(tri, te, List.of(0,1,2));
            if (relMin.size() == 1) return edgeEvt(tri, te, List.of(relMin.get(0)));
        ...
        List<Integer> zeros = new ArrayList<>();
        for (int i=0; i<3; i++) if (nearZero(d2[i])) zeros.add(i);
        if (zeros.size() == 3) return edgeEvt(tri, te, List.of(0,1,2));
        if (zeros.size() == 1) return edgeEvt(tri, te, List.of(zeros.get(0)));
    ...
    private static Event finite1(KineticTriangle tri, double now, boolean strictGt) {
        ...
        if (tc != null && nearZero(Math.abs(tc - now))) {
            ...
            for (int i = 0; i < 3; i++) {
                if (nearZero(Math.sqrt(d2Now[i]))) zeroLenSides.add(i);
            }
        ...
        if (te == null && tc == null) {
            ...
            if (nearZero(ta - now)) return splitEvt(tri, now, wfSide);
            double[] d2 = sideD2(o, d, a, ta);
            for (int i=0; i<3; i++) if (tri.neighbours[i] == null) d2[i] = -1.0;
            return flipEvt(tri, ta, d2[0] > d2[1] ? (d2[0] > d2[2] ? 0 : 2) : (d2[1] > d2[2] ? 1 : 2));
        }
        if (te == null && tc != null) {
            ...
            for (int i=0; i<3; i++) if (nearZero(Math.sqrt(d2[i]) - Math.sqrt(mx))) longs.add(i);
            if (longs.size() == 1 && longs.get(0) == wfSide) return splitEvt(tri, tc, wfSide);
            int zCt = 0, zIdx = -1;
            for (int i=0; i<3; i++) if (nearZero(Math.sqrt(d2[i]))) { zCt++; zIdx = i; }
            if (zCt == 1) return edgeEvt(tri, t, List.of(zIdx));
            return flipEvt(tri, t, d2[0] > d2[1] ? (d2[0] > d2[2] ? 0 : 2) : (d2[1] > d2[2] ? 1 : 2));
        }
        if (te != null && tc == null) return edgeEvt(tri, te, List.of(wfSide));
        if (te <= tc) {
            double[] d2 = sideD2(o, d, a, te);
        ...
        int zCt = 0, zIdx = -1;
        for (int i=0; i<3; i++) if (nearZero(Math.sqrt(d2[i]))) { zCt++; zIdx = i; }
        if (zCt == 1) return edgeEvt(tri, tc, List.of(zIdx));
        if (zCt == 3) return edgeEvt(tri, tc, List.of(0,1,2));
        int mx = d2[0] > d2[1] ? (d2[0] > d2[2] ? 0 : 2) : (d2[1] > d2[2] ? 1 : 2);
    ...
    private static Event finite2(KineticTriangle tri, double now, boolean strictGt) {
        ...
        for (int i=0; i<3; i++) if (tri.neighbours[i] == null) tms.add(collapseTimeEdge(i==0?d:(i==1?a:o), i==0?a:(i==1?o:d), now));
        Double t = strictGt ? findGt(getUniqueTimes(tms), now) : findGte(getUniqueTimes(tms), now);
        if (t == null) t = strictGt ? findGt(areaCollapseTimes(o, d, a, now), now) : findGte(areaCollapseTimes(o, d, a, now), now);
        if (t == null) return null;
        double[] l = sideD2(o, d, a, t);
        for(int i=0; i<3; i++) l[i] = Math.sqrt(l[i]);
        double mn = Math.min(l[0], Math.min(l[1], l[2]));
        int zCt = 0, zIdx = -1;
        for (int i=0; i<3; i++) if (nearZero(l[i] - mn)) { zCt++; zIdx = i; }
        if (zCt == 3) return edgeEvt(tri, t, List.of(0,1,2));
        if (zCt == 1) return edgeEvt(tri, t, List.of(zIdx));
        if (zCt == 2) throw new IllegalStateException("2-triangle: impossible configuration (two sides are equal-min)");
    ...
    private static Event finite3(KineticTriangle tri, double now, boolean strictGt) {
        ...
        for (Double t : tE) if (t != null) edgeTimes.add(t);
        ...
        if (tEdge != null) {
            ...
            for (int i=0; i<3; i++) if (nearZero(Math.sqrt(d2AtTe[i]))) idx.add(i);
            List<Integer> sides = idx.isEmpty() ? List.of(0,1,2) : idx;
            if (sides.size() == 2 || sides.isEmpty()) sides = List.of(0,1,2);
            return edgeEvt(tri, tEdge, sides);
        }
        if (tArea != null) return edgeEvt(tri, tArea, List.of(0,1,2));
        return null;
    }
    ...
    public static Event computeNewEdgeCollapse(KineticTriangle tri, double time) {
        ...
        for (int i=0; i<3; i++) if (nearZero(l[i] - mn)) sides.add(i);
